package com.pwc.testcase.submission

uses com.pwc.policyBuilder.PolicyBuilderMain
uses gw.api.util.DateUtil
uses gw.testharness.TestBase

class BA_SubmissionTest extends TestBase {

  function testValidateVehicles() {
    var polPeriod = new PolicyBuilderMain().createBAPolicy()
    assertTrue(polPeriod.BusinessAutoLine.Vehicles.HasElements)
  }

  function testValidateFleetType() {
    var pb = new PolicyBuilderMain()
    var polPeriod = pb.createBAPolicy()
    var prop = pb.getBAProperties()
    assertTrue(polPeriod.BusinessAutoLine.Fleet == FleetType.get(prop.getProperty("vehicleFleetType")))
  }

  function testDriverAge() {
    var policyPeriod = PolicyBuilderMain.createBAPolicy()
